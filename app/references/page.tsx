...
const categories: ReferenceCategory[] = [
  ...
  },
  {
    ...
    adminCanCreate: false,
    color:
      "bg-success/15 text-success dark:bg-navy-600 dark:text-success-light",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="size-7"
  ...
  return (
    <DashboardLayout theme="admin">
      ...
      <div className="py-5 lg:py-6">
        <h2 className="text-xl font-medium text-slate-700 dark:text-navy-50">
          Reference Data
        </h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {visibleCategories.map((cat) => (
          ...
          >
            <div
              className={`flex size-12 shrink-0 items-center justify-center rounded-xl ${cat.color}`}
            >
              {cat.icon}
            </div>
