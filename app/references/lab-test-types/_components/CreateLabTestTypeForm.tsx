...
interface CreateLabTestTypeFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateLabTestTypeRequest) => Promise<void>;
  error: string | null;
  isSubmitting: boolean;
}

export default function CreateLabTestTypeForm({
  isOpen,
  onClose,
  onSubmit,
  error,
  isSubmitting,
}: CreateLabTestTypeFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center px-4" role="dialog">
      ...
      <div className="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-navy-700">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4 dark:border-navy-500">
          <h3 className="text-lg font-semibold text-slate-700 dark:text-navy-100">Add Lab Test Type</h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is_active" className="text-sm font-medium text-slate-700 dark:text-navy-100 cursor-pointer">
              Active
            </label>
          </div>
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            >
              {isSubmitting ? "Creating..." : "Create Lab Test Type"}
            </button>
          </div>
        </form>
      </div>
