...
export default function ProfileContent() {
  ...
  const { viewedUser, setViewedUser, isLoading: isLoadingProfile, fetchViewedUser, clearViewedUser } = useProfileStore();
  ...
  // Fetch via shared store — ProfileSidebar uses the same store so only one request fires
  useEffect(() => {
    fetchViewedUser(userId, user?.id).then((fetched) => {
      // Sync the auth store with fresh own-profile data
      if (fetched && (!userId || userId === user?.id)) {
        setUser(fetched);
      }
    });
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      if (response.data && response.data.url && viewedUser) {
        const updatedUser = { ...viewedUser, profilePictureUrl: response.data.url };
